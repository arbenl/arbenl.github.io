...
export async function POST(request: Request) {
  ...
  const input = z.object({ week: z.number().int().min(1).max(15), kind: z.enum(["lecture", "lab"]) })
    .safeParse(await request.json().catch(() => null));
  if (!input.success) return Response.json({ error: { message: "Ora nuk është e vlefshme." } }, { status: 400 });
  try {
    const session = await attendanceService.launchCourseSession(await getCurrentSession(), input.data.week, input.data.kind);
