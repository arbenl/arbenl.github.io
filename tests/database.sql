...
set role authenticated;
select public.assert_ok(public.attendance_api('scan',jsonb_build_object('token',:'token'))->>'status'='pending','QR creates pending only');
...
set role authenticated;
do $$begin
 begin perform public.attendance_api('state',jsonb_build_object('session_id',current_setting('test.sid'),'state','finalized'));raise exception 'FAIL finalized pending';exception when raise_exception then if sqlerrm like 'FAIL%' then raise;end if;raise notice 'PASS pending prevents finalization';end;
end$$;
reset role;
